...
/* =====================================================================
   ...
   Uso en cPanel (Terminal), dentro de la carpeta del proyecto:
       php artisan tinker metricas.php > metricas.txt
...
tabla(DB::select("SELECT MIN(fecha_creacion) primer_turno, MAX(fecha_creacion) ultimo_turno,
  COUNT(*) total_turnos, COUNT(DISTINCT DATE(fecha_creacion)) dias_operacion FROM turnos"));

seccion('2. TURNOS POR ESTADO');
tabla(DB::select("SELECT estado, COUNT(*) cantidad,
...
tabla(DB::select("SELECT DATE_FORMAT(fecha_creacion,'%Y-%m') mes, COUNT(*) turnos,
...
tabla(DB::select("SELECT COUNT(*) turnos_medidos, ROUND(AVG(ABS(duracion_atencion)),1) promedio,
...
tabla(DB::select("SELECT COUNT(*) turnos_medidos,
...
tabla(DB::select("SELECT s.nombre servicio, COUNT(t.id) total, SUM(t.estado='atendido') atendidos,
...
tabla(DB::select("SELECT u.nombre_completo asesor, COUNT(t.id) atendidos,
...
tabla(DB::select("SELECT HOUR(fecha_creacion) hora, COUNT(*) turnos FROM turnos GROUP BY hora ORDER BY hora"));

seccion('9. PRIORIDAD');
tabla(DB::select("SELECT prioridad, COUNT(*) cantidad FROM turnos GROUP BY prioridad ORDER BY prioridad"));

seccion('10. TRANSFERENCIAS');
tabla(DB::select("SELECT SUM(observaciones LIKE 'Transferido a%') salientes,
  SUM(observaciones LIKE 'Transferido desde%') recibidos, COUNT(*) total FROM turnos"));

seccion('11. INFRAESTRUCTURA');
tabla(DB::select("SELECT (SELECT COUNT(*) FROM servicios) servicios,
...
tabla(DB::select("SELECT DATE(fecha_creacion) dia, COUNT(*) turnos FROM turnos
