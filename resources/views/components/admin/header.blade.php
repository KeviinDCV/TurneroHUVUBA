...
<header class="header-responsive px-5 md:px-6 py-3 flex items-center justify-between fixed top-0 left-0 md:left-72 right-0 z-20" style="background: #072449;">
    <div class="flex items-center gap-3 min-w-0">
        ...
        <button @click="sidebarOpen = !sidebarOpen" :aria-expanded="sidebarOpen" aria-controls="admin-sidebar" aria-label="Abrir menú de navegación" class="md:hidden text-white hover:bg-white/10 p-2 -ml-2 rounded-lg transition-colors flex-shrink-0">
            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
            </svg>
        </button>
        <p class="text-lg font-semibold text-white truncate">@yield('title', 'Panel administrativo')</p>
    </div>
